...
%>
	<div class = "container">
		...
		<div class = "text-right">
			<a href="<%=request.getContextPath()%>/jsp_board/boardAddForm.jsp" class="btn btn-outline-primary"> 게시글 작성 </a>
		</div>
		현재 페이지 : 
		<%= currentPage%>
		<br>
...
%>
			<tr>
				<td><a
					href="<%=request.getContextPath()%>/jsp_board/boardView.jsp?boardNo=<%=listResultSet.getInt("board_no")%>"><%=listResultSet.getString("board_title")%></a></td>
...
%>				
				<a href="<%=request.getContextPath()%>/jsp_board/boardList.jsp?currentPage=<%=currentPage - 1%>" class="btn btn-outline-primary" >이전</a>
<%	
			}
				if (currentPage < lastPage) { // 현재 페이지가 마지막 페이지보다 작으면 다음페이지 링크를 추가
%>
					<a href="<%=request.getContextPath()%>/jsp_board/boardList.jsp?currentPage=<%=currentPage + 1%>" class="btn btn-outline-primary">다음</a>
